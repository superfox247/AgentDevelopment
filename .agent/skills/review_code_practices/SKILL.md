...
1.  **Full Stack Scan**:
    -   **Frontend**: `package.json` (Frameworks, UI Libs).
    -   **Backend/Scripting**: `requirements.txt`, `pyproject.toml` (FastAPI, Pydantic, etc.).
    -   **Infrastructure**: `Dockerfile`, `docker-compose.yml` (System dependencies, OS versions).
    -   **Internal Frameworks**: Look for repeated imports (e.g., `from adk import agent`). These are "Local Standards".
...
3.  **Create Master Checklist**:
    -   Create/Update `review_checklist.md`.
...
4.  **Review Loop**:
    -   **Select**: Pick a batch of items from `review_checklist.md`.
    -   **Context Loading**: Load relevant `patterns/*.md`.
    -   **Analyze & Verify**: Check code against patterns.
    -   **🚨 Ad-Hoc Research Trigger**:
        -   *Trigger*: You see a pattern usage you are unsure about (e.g., "Is this the right way to use `useEffect` in v19?" or "How does `adk.Agent` handle streams?").
        -   *Action*: **STOP**. Run a search or `grep_search` to find the answer. **UPDATE** the relevant `patterns/*.md`. **RESUME**.
    -   **Record**: 
