        ...
        try:
            with urllib.request.urlopen(url, timeout=2) as response:
                if response.status < 400:
                    ...
                    logger.warning(
                        f"  ⚠️  {name:<20} | {url} | Status {response.status}"
                    )
...
def rebuild_service(target):
    ...
    _stdout, stderr, code = run_command(cmd)
...
def attempt_fix():
    ...
    for service in services:
        ...
        if state in ["exited", "dead"]:
            logger.info(f"  🔄 Force-recreating dead service: {name}")
            # Use Force Recreate (Clean restart) as requested
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target", default="all", help="Target service to analyze (or 'all')"
    )
    parser.add_argument(
        "--fix", action="store_true", help="Attempt to fix common issues"
    )
    parser.add_argument(
        "--rebuild", help="Target service to rebuild and force-recreate (or 'all')"
    )
