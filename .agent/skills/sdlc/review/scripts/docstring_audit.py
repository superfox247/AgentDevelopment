...
def main():
    ...
    for file_path in sampled_files:
        ...
        class_stats = (
            f"{res['documented_classes']}/{res['total_classes']}"
            if res["total_classes"] > 0
            else "-"
        )
        func_stats = (
            f"{res['documented_funcs']}/{res['total_funcs']}"
            if res["total_funcs"] > 0
            else "-"
        )
    ...
    print(
        f"Module Docstring Coverage: {len(results) - missing_module_docs}/{len(results)} ({((len(results) - missing_module_docs) / len(results)) * 100:.1f}%)"
    )

    if total_classes > 0:
        print(
            f"Class Docstring Coverage:  {total_documented_classes}/{total_classes} ({(total_documented_classes / total_classes) * 100:.1f}%)"
        )
    else:
    ...
    if total_funcs > 0:
        print(
            f"Func Docstring Coverage:   {total_documented_funcs}/{total_funcs} ({(total_documented_funcs / total_funcs) * 100:.1f}%)"
        )
    else:
