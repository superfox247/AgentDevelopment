...
def verify_dashboard():
    ...
    try:
        ...
        subprocess.run(
            ["npm", "run", "lint"], cwd=DASHBOARD_DIR, check=True, shell=True
        )

        # 2. Build
        logger.info("   Running Vite Build...")
        subprocess.run(
            ["npm", "run", "build"], cwd=DASHBOARD_DIR, check=True, shell=True
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Automated skill for building and extending the Antigravity Dashboard."
    )

    parser.add_argument(
        "--component",
        type=str,
        help="Name of the new component to create (e.g. SettingsView)",
    )
    parser.add_argument(
        "--verify", action="store_true", help="Run lint and build verification"
    )
