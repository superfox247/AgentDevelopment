...
import { ArtifactInfo } from '../api/schemas';
            ...
            {artifacts.map((art) => (
                <div key={art.path} className="glass-card rounded-xl overflow-hidden group relative">
                    <div className="aspect-square bg-black/20 flex items-center justify-center">
                        {art.type === 'image' ? (
                            <img
                                src={`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8010/api'}/artifacts/${art.path}`}
                                alt={art.name}
                    ...
                    <a
                        href={`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8010/api'}/artifacts/${art.path}`}
                        target="_blank"
