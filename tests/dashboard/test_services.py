...
@pytest.mark.asyncio
async def test_generate_image_success_json(
    service: ImageGenerationService, fake_runner: FakeRunner
) -> None:
...
@pytest.mark.asyncio
async def test_generate_image_success_tool(
    service: ImageGenerationService, fake_runner: FakeRunner
) -> None:
    ...
    event = FakeEvent(tool_response=[fake_tool_resp])
    fake_runner.set_mock_events([event])
...
@pytest.mark.asyncio
async def test_generate_image_failure_no_path(
    service: ImageGenerationService, fake_runner: FakeRunner
) -> None:
...
@pytest.mark.asyncio
async def test_generate_image_from_content_parts(
    service: ImageGenerationService, fake_runner: FakeRunner
) -> None:
    ...
    event = FakeEvent(content=FakeContent(parts=[fake_part]))
    fake_runner.set_mock_events([event])
