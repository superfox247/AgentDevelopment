...
@pytest.mark.asyncio
async def test_generate_image_routing_imagen(
    img_service, mock_genai_client, mock_persistence
):
    ...
    mock_response = types.GenerateImagesResponse(
        generated_images=[
            types.GeneratedImage(image=types.Image(image_bytes=b"fake_imagen_bytes"))
        ]
    )
    mock_genai_client.aio.models.generate_images.return_value = mock_response

    # Execute
    result = await img_service.generate_image_from_prompt(
        "test prompt", model="imagen-3.0-generate-001"
    )

    # Verify
    mock_genai_client.aio.models.generate_images.assert_called_once()
    mock_persistence.save_image.assert_called_once_with(
        b"fake_imagen_bytes", "test prompt", "imagen-3.0-generate-001"
    )
...
@pytest.mark.asyncio
async def test_generate_image_routing_gemini_default(
    img_service, mock_genai_client, mock_persistence
):
    """Verify that default model (non-imagen) routes to the Gemini handler."""
    # Setup - Gemini Logic Mocking
    mock_part = types.Part(
        inline_data=types.Blob(mime_type="image/png", data=b"fake_gemini_bytes")
    )
    ...
    mock_persistence.save_image.assert_called_once_with(
        b"fake_gemini_bytes", "test prompt", "gemini-2.0-flash-exp"
    )
    assert result == "/path/to/generated_image.png"
...
async def test_imagen_failure_empty_bytes(img_service, mock_genai_client):
    ...
    mock_response = types.GenerateImagesResponse(
        generated_images=[types.GeneratedImage(image=types.Image(image_bytes=None))]
    )
    mock_genai_client.aio.models.generate_images.return_value = mock_response
