...
import { Terminal, RefreshCw, Box, AlertCircle, Pause, Play, Download } from 'lucide-react';
                    ...
                    <div className="overflow-y-auto flex-1 p-2 space-y-1 custom-scrollbar">
                        {containers.map(c => (
                            ...
                            >
                                ...
                                <span className="truncate">{c.name.replace('course_creator-', '')}</span>
                            </button>
                        ))}
                    </div>
                </div>
...
function LogStreamer({ container }) {
            ...
            try {
                ...
                if (payload.text) {
                    setLogs(prev => {
                        ...
                        const newState = [...prev, { id: crypto.randomUUID(), text: payload.text, type: 'stdout' }];
                        if (newState.length > 2000) return newState.slice(newState.length - 2000);
                        return newState;
                    });
                }
            } catch (e) {
