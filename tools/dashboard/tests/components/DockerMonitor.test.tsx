...
const mockContainers = {
    containers: [
        {
            id: 'abc123',
            name: 'course_creator-orchestrator',
            status: 'Up 2 hours',
            image: 'orchestrator:latest',
        },
        {
            id: 'def456',
            name: 'course_creator-image_gen',
            status: 'Exited (0)',
...
describe('DockerMonitor', () => {
    ...
    it('displays container names', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText(/course_creator-orchestrator/i)).toBeInTheDocument();
            expect(screen.getByText(/course_creator-image_gen/i)).toBeInTheDocument();
        });
    });
