...
def scaffold_agent(agent_name: str, domain: str, role: str, heuristics: str, verification: str):
    # Normalize names
    ...
    print(f"Scaffolding agent '{name}' in '{domain}'...")
    
    # 1. Ensure domain exists
    if not domain_dir.exists():
        print(f"Creating domain directory: {domain_dir}")
        domain_dir.mkdir(parents=True, exist_ok=True)
        (domain_dir / "__init__.py").touch()
        
    # 2. Create agent directory
    if agent_dir.exists():
        print(f"Warning: Agent directory already exists: {agent_dir}")
    else:
    ...
    # 4. Create agent.yaml
    yaml_content = AGENT_YAML_TEMPLATE.format(name=agent_name, name_title=agent_name.replace("_", " ").title())
...
"""
    ...
    print("Next steps:")
    print("1. Edit agent.yaml to define tools.")
    print("2. Edit instruction.md to refine logic.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--heuristics", default="TODO: Define when this agent should be called.", help="Cognitive Policy")
    parser.add_argument("--verification", default="TODO: Define how to verify this agent's output.", help="Verification Logic")
    
    args = parser.parse_args()
    
    scaffold_agent(args.name, args.domain, args.role, args.heuristics, args.verification)
